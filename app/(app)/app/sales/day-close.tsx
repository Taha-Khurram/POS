...
import type { CounterTakings, DayTakings } from "@/lib/pos/takings";
 ...
 *
 * It lists no receipts. It used to, and that table is now the Bills tab, which
 * searches, pages and reprints — two lists of the same rows on one screen is
 * how they drift apart. What is left here is the drawer count, and one link
 * into the other tab already narrowed to this day.
 *
 * The day lives in the URL (`?tab=day&day=2026-09-16`), like every other window
 * in the console, so the page stays a server component and last Tuesday can be
 * sent to an accountant as a link. It is a trading day off `sales.business_day`,
 * not a calendar day — a dhaba that shuts at 1 am gets its last hour on the day
 * it opened.
 */
    ...
    <div className="space-y-4">
      <header className="flex flex-wrap items-end justify-between gap-x-4 gap-y-2">
        <div className="min-w-0">
          <h2 className="font-display text-[1.0625rem] leading-tight font-bold">
            {writeBusinessDay(day)}
          </h2>
          <p className="mt-0.5 text-[0.8125rem] text-graphite-500">
            {day === currentBusinessDay(settings)
              ? "Today so far. Count a drawer against its own row."
              : "Count a drawer against its own row."}
          </p>
        </div>
      ...
      {/* The way out of a total and into the bills behind it. Narrowed to this
          same trading day, so the two tabs can never be showing different
          days at once. */}
      <p className="text-[0.8125rem] text-graphite-500">
        <Link
          href={`/app/sales?tab=history&range=custom&from=${day}&to=${day}`}
          className="inline-flex items-center gap-1 font-medium text-orchid-700 underline-offset-2 hover:underline"
          scroll={false}
        >
          See every bill on this day
 ...
 * that is opened at closing time about the day that is closing. The Bills tab's
...
function DayStep({ day, settings }: { day: string; settings: ShopSettings }) {
  ...
  return (
    <nav className="flex items-center gap-1.5" aria-label="Trading day">
      <Link
        href={`/app/sales?tab=day&day=${shift(-1)}`}
      ...
      {day !== today ? (
        <Link href="/app/sales?tab=day" className="pos-btn pos-btn-soft pos-btn-sm" scroll={false}>
          Today
        </Link>
      ) : null}
      ...
      {next <= today ? (
        <Link
          href={`/app/sales?tab=day&day=${next}`}
          className="pos-btn pos-btn-soft pos-btn-sm"
          scroll={false}
        >
    ...
    >
      {tiles.map((tile) => (
        <article key={tile.label} className="pos-card p-4">
          <h3 className="flex items-center gap-1.5 font-display text-[0.8125rem] leading-tight font-semibold text-graphite-500">
  ...
  return (
    <section className="pos-card">
      <header className="px-4 pt-4 pb-3">
        <h3 className="font-display text-[0.9375rem] leading-tight font-semibold">
          Each counter
        </h3>
